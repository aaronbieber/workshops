# This file is auto-generated from the current state of the database. Instead of editing this file, 
# please use the migrations feature of Active Record to incrementally modify your database, and
# then regenerate this schema definition.
#
# Note that this schema.rb definition is the authoritative source for your database schema. If you need
# to create the application database on another system, you should be using db:schema:load, not running
# all the migrations from scratch. The latter is a flawed and unsustainable approach (the more migrations
# you'll amass, the slower it'll run and the greater likelihood for issues).
#
# It's strongly recommended to check this file into your version control system.

ActiveRecord::Schema.define(:version => 20100125175307) do

  create_table "administrators", :force => true do |t|
    t.string   "email",       :limit => 64, :default => "", :null => false
    t.string   "pass",        :limit => 40, :default => "", :null => false
    t.string   "salt",        :limit => 10, :default => "", :null => false
    t.datetime "created_at",                                :null => false
    t.datetime "modified_at",                               :null => false
  end

  create_table "pages", :force => true do |t|
    t.string   "title",       :limit => 128, :default => "", :null => false
    t.string   "name",        :limit => 128, :default => "", :null => false
    t.text     "text"
    t.datetime "created_at",                                 :null => false
    t.datetime "modified_at"
  end

  create_table "registrants", :force => true do |t|
    t.integer "student_id"
    t.integer "workshop_id"
    t.date    "payment_submitted"
    t.date    "payment_received"
    t.text    "comments"
  end

  create_table "students", :force => true do |t|
    t.string   "first_name",        :limit => 64,  :default => "", :null => false
    t.string   "last_name",         :limit => 64,  :default => "", :null => false
    t.string   "email",             :limit => 128, :default => "", :null => false
    t.string   "address1",          :limit => 128, :default => "", :null => false
    t.string   "address2",          :limit => 128
    t.string   "city",              :limit => 128, :default => "", :null => false
    t.string   "state",             :limit => 2,   :default => "", :null => false
    t.string   "zip",               :limit => 9,   :default => "", :null => false
    t.string   "phone",             :limit => 10
    t.string   "ext",               :limit => 10
    t.datetime "created_at",                                       :null => false
    t.datetime "modified_at"
    t.string   "pass",              :limit => 40,                  :null => false
    t.string   "salt",              :limit => 10
    t.string   "new_password",      :limit => 32
    t.date     "new_password_date"
  end

  create_table "workshops", :force => true do |t|
    t.string   "name",         :limit => 128, :default => "",    :null => false
    t.string   "short_name",   :limit => 128, :default => "",    :null => false
    t.text     "description",                                    :null => false
    t.date     "cutoff_date",                                    :null => false
    t.date     "start_date",                                     :null => false
    t.date     "end_date",                                       :null => false
    t.integer  "max_students"
    t.float    "cost"
    t.datetime "created_at",                                     :null => false
    t.datetime "modified_at"
    t.boolean  "active",                      :default => false
    t.string   "thumbnail"
    t.text     "short_desc"
    t.boolean  "full",                        :default => false
    t.integer  "ancestor_id"
  end

end
